...
out_path1 = r'D:\UMA_Dataset\Accelerometer_Wrist_User1'
out_path2 = r'D:\UMA_Dataset\Gyroscope_Wrist_User1'
out_path3 = r'D:\UMA_Dataset\Magnetometer_Wrist_User1'
...
for i in range(0,len(os.listdir(path=input_path))):
    ...
    #之後去判斷拆解說 這個split裡面是啥 去抓感測器和腰部一些分類
        if(df.iloc[index,0].split(';')[5] == '0' and df.iloc[index,0].split(';')[6] == '3'):
            data = {
    ...
    output_file = os.path.join(out_path1,'Accelerometer_wrist'+str(i)+'.csv')            
...
for i in range(0,len(os.listdir(path=input_path))):
    ...
    #df.iloc[index,0].split(';')[5] 是做感測器的分類
    #之後去判斷拆解說 這個split裡面是啥 去抓感測器和腰部一些分類
        if(df.iloc[index,0].split(';')[5] == '1' and df.iloc[index,0].split(';')[6] == '3'):
            data = {
            'TimeStamp': int(df.iloc[index, 0].split(';')[0]),  # 轉換為整數
            'Sample No': int(df.iloc[index, 0].split(';')[1]),  # 轉換為整數
            'X-Axis': float(df.iloc[index, 0].split(';')[2]),  # 轉換為浮點數
    ...
    output_file = os.path.join(out_path2,'Gyroscope_wrist'+str(i)+'.csv')            
    filtered_data.to_csv(output_file, index=False,sep=',')
...
for i in range(0,len(os.listdir(path=input_path))):
    ...
    #df.iloc[index,0].split(';')[5] 是做感測器的分類
    #之後去判斷拆解說 這個split裡面是啥 去抓感測器和腰部一些分類
        if(df.iloc[index,0].split(';')[5] == '2' and df.iloc[index,0].split(';')[6] == '3'):
            data = {
            'TimeStamp': int(df.iloc[index, 0].split(';')[0]),  # 轉換為整數
            'Sample No': int(df.iloc[index, 0].split(';')[1]),  # 轉換為整數
            'X-Axis': float(df.iloc[index, 0].split(';')[2]),  # 轉換為浮點數
    ...
    output_file = os.path.join(out_path3,'Magnetometer_wrist'+str(i)+'.csv')            
    filtered_data.to_csv(output_file, index=False,sep=',')           
